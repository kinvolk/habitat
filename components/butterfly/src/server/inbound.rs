...
impl<N: Network> Inbound<N> {
    ...
    fn handle_zone_for_recipient(
        ...
        let mut from_address_kind = Self::address_kind(addr.get_address(), from, &mut dbg_data);
        let mut to_address_kind = Self::address_kind_from_str(
            to.get_address(),
            &self.server.read_member(),
            &mut dbg_data,
        );
        let to_clone: Option<ProtoMember>;
        let mut to_ref = to;
        ...
        println!("address kinds before fix: from {:?}, to {:?}", from_address_kind, to_address_kind);
        println!("before fix, addr: {}, from: {:#?}, to: {:#?}", addr, from, to_ref);
        match (from_address_kind, to_address_kind) {
            ...
            (AddressKind::Additional, AddressKind::Unknown) => {
                ...
                sender_in_the_same_zone_as_us = false;
                let mut tc = to.clone();
                tc.set_address(self.server.host_address.to_string());
                to_clone = Some(tc);
            ...
            (AddressKind::Unknown, AddressKind::Additional) => {
                ...
                // inside by default.
                match Self::parse_addr(from.get_address()) {
                    Ok(a) => {
                        addr = N::AddressAndPort::new_from_address_and_port(a, from.get_swim_port() as u16);
                        from_address_kind = AddressKind::Real;
                    }
                    Err(e) => {
                        error!("Could not parse from address {}: {}", from.get_address(), e);
        ...
        println!("address kinds after fix: from {:?}, to {:?}", from_address_kind, to_address_kind);
        println!("after fix, addr: {}, from: {:#?}, to: {:#?}", addr, from, to_ref);

        dbg_data.to_address = to_ref.get_address().to_string();
        dbg_data.to_port = to_ref.get_swim_port() as u16;
