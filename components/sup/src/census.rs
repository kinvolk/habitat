...
use butterfly::message::{BfUuid, swim::{NamedPort, ZoneAddress}};
...
use butterfly::zone::{Reachable, ZoneList};
...
impl CensusRing {
    ...
    pub fn update_from_rumors(
        ...
        {
            outputln!(
                "Updating census ring\n\
                 service rumors: {} > {}\n\
                 member list: {} > {}\n\
                 election rumors: {} > {}\n\
                 election update rumors: {} > {}\n\
                 service config rumors: {} > {}\n\
                 service file rumors: {} > {}\n\
                 zone list: {} > {}",
                service_rumors.get_update_counter(), self.last_service_counter,
                member_list.get_update_counter(), self.last_membership_counter,
                election_rumors.get_update_counter(), self.last_election_counter,
                election_update_rumors.get_update_counter(), self.last_election_update_counter,
                service_config_rumors.get_update_counter(), self.last_service_config_counter,
    ...
    fn populate_census(
        ...
        let our_zone_id = member_list
            ...
            .map(|m| BfUuid::parse_or_nil(m.get_zone_id(), "our zone ID"))
...
impl CensusGroup {
    ...
    fn update_from_service_rumors_members_and_zones_local_only(
        ...
        for (member_id, service_rumor) in rumors.iter() {
            ...
            match self.population.entry(member_id.to_string()) {
                Entry::Occupied(oe) => {
                    oe.into_mut().update_from_service_rumor(&self.service_group, service_rumor, zone_address, named_ports, health);
                }
                Entry::Vacant(ve) => {
                    let mut new_member = CensusMember::default();

                    new_member.update_from_service_rumor(&self.service_group, service_rumor, zone_address, named_ports, health);
    ...
    fn update_from_service_rumors_members_and_zones_all(
        ...
        let our_zone_id = our_zone_uuid.to_string();
        let members = member_list.members.read().expect("Members lock is poisoned");
        ...
        for (member_id, service_rumor) in rumors.iter() {
            ...
            let service_member_zone_id = BfUuid::parse_or_nil(service_member.get_zone_id(), "zone ID of a member from service rumor");
            ...
            let reachable = zone_list.directly_reachable(
                &our_zone_id,
                service_member.get_zone_id(),
                our_member.get_additional_addresses(),
                service_member.get_additional_addresses(),
            );
            let zone_address = match reachable {
                ...
                Reachable::ThroughOtherZone(zone_id) => {
                    ...
                    for zone_address in service_member.get_additional_addresses() {
                ...
                Reachable::No => {
                    outputln!("Skippping updating census from a service rumor - service is unreachable");
                    self.population.remove(member_id);
                    continue;
                }
            };
            let named_ports = {
                if let Some(ref za) = zone_address {
                    let tag = za.get_tag();
                    let mut our_named_ports = None;

                    for tagged_ports in service_rumor.get_ports().iter() {
                        if tag == tagged_ports.get_tag() {
                            our_named_ports = Some(tagged_ports.get_named_ports());
                            break;
                        }
                    }
...
impl CensusMember {
    ...
    fn setup_sys(&mut self, rumor: &ServiceRumor, zone_address: &Option<&ZoneAddress>) {
        ...
        if let Some(ref za) = zone_address {
            self.sys.set_ip(za.get_address().to_string());
            // TODO(krnowak): What about hostname? Clear it?
            self.sys.set_gossip_ip(za.get_address().to_string());
            self.sys.set_gossip_port(za.get_gossip_port() as u32);
            // TODO(krnowak): What about http/ctl gateway address and port?
        }
    }

    fn setup_cfg(&mut self, rumor: &ServiceRumor, named_ports: &Option<&[NamedPort]>) {
        self.cfg = toml::from_slice(rumor.get_cfg()).unwrap_or(toml::value::Table::default());
        if let Some(np) = named_ports {
            for named_port in np.iter() {
                if let Some(ref mut value) = self.cfg.get_mut(named_port.get_name()) {
                    match value {
                        toml::value::Value::String(ref mut s) => {
                            *s = format!("{}", named_port.get_port());
                        }
                        toml::value::Value::Integer(ref mut i) => {
                            *i = named_port.get_port() as i64;
                        }
                        _ => {
                            // TODO(krnowak): Warn?
                        }
...
mod tests {
    ...
    fn update_from_rumors() {
        ...
        let service_one =
            ServiceRumor::new("member-a".to_string(), &pg_id, &sg_one, &sys_info, None, &tagged_ports);
        let sg_two = ServiceGroup::new(None, "shield", "two", None).unwrap();
        let service_two =
            ServiceRumor::new("member-b".to_string(), &pg_id, &sg_two, &sys_info, None, &tagged_ports);
        let service_three =
            ServiceRumor::new("member-a".to_string(), &pg_id, &sg_two, &sys_info, None, &tagged_ports);
        ...
        member_b.set_id("member-b".to_string());
        member_b.set_zone_id(zone_uuid.to_string());
    ...
    fn update_from_rumors_from_different_zone() {
        let mut sys_info_inside = SysInfo::new();
        sys_info_inside.set_ip("4.3.2.1".to_string());
        sys_info_inside.set_hostname("inside".to_string());
        sys_info_inside.set_gossip_ip("0.0.0.0".to_string());
        sys_info_inside.set_gossip_port(7777);
        sys_info_inside.set_http_gateway_ip("0.0.0.0".to_string());
        sys_info_inside.set_http_gateway_port(9631);
        ...
        cfg.insert("ssl-port".to_string(), toml::value::Value::String("4321".to_string()));
        ...
        let mut zone_outside = Zone::new(zone_uuid_outside.to_string(), "member-outside".to_string());
        let mut zone_inside = Zone::new(zone_uuid_inside.to_string(), "member-inside".to_string());

        zone_outside.mut_child_zone_ids().push(zone_uuid_inside.to_string());
        zone_inside.set_parent_zone_id(zone_uuid_outside.to_string());

        let mut member_outside = Member::default();

        member_outside.set_id("member-outside".to_string());
        member_outside.set_zone_id(zone_uuid_outside.to_string());

        let mut member_inside = Member::default();
        let mut zone_address = ZoneAddress::new();

        zone_address.set_zone_id(zone_uuid_outside.to_string());
        zone_address.set_address("1.2.3.4".to_string());
        zone_address.set_swim_port(33333);
        zone_address.set_gossip_port(44444);
        zone_address.set_tag("tag".to_string());

        member_inside.set_id("member-inside".to_string());
        member_inside.set_zone_id(zone_uuid_inside.to_string());
        member_inside.mut_additional_addresses().push(zone_address);
        ...
        assert_eq!(census_group.update_leader().unwrap().member_id, "member-inside");
        ...
        assert_eq!(test_member_inside.sys.get_ip(), "1.2.3.4");
        // same for gossip ip
        assert_eq!(test_member_inside.sys.get_gossip_ip(), "1.2.3.4");
        // same for gossip port
        assert_eq!(test_member_inside.sys.get_gossip_port(), 44444);
