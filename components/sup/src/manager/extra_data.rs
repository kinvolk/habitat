...
impl AdditionalAddresses {
    ...
    pub fn create_from_toml_string(s: &str) -> Result<Self> {
        let aa_to_parse: AdditionalAddressesToParse = toml::from_str(s)
...
impl AdditionalAddressesWatcher {
    ...
    where
        P: Into<PathBuf>,
    {
        Ok(AdditionalAddressesWatcher(SimpleFileWatcher::run("additional-addresses-watcher".to_string(), path)?))
    }
    ...
    pub fn get_additional_addresses(&self) -> Result<AdditionalAddresses> {
        ...
        file.read_to_string(&mut contents).map_err(|e| sup_error!(Error::Io(e)))?;
